...
def insert_test_users():
    ...
    # List of test image URLs for users
    user_image_urls = [
...
# Function to insert test categories
...
# Function to insert test ingredients
...
# Function to insert test recipes
def insert_test_recipes():
    print('\n\n--- Add test RECIPES | START ---')
    
    # Get some ingredients and categories from the database
    ingredients = Ingredients.query.all()[:3]  # Limit to 3 ingredients for simplicity
    categories = Categories.query.all()[:2]    # Limit to 2 categories for simplicity
    
    # Get users to associate with recipes
    users = User.query.all()[:3]
    
    # List of test image URLs (can modify URLs)
    image_urls = [
    ...
    # Create test recipes
    for i in range(7):  # Create 7 test recipes
        recipe = Recipe(
            name=f"Recipe {i + 1}",
            description=f"This is the description for Recipe {i + 1}",
            steps="Step 1: Do this, Step 2: Do that",
            user=users[i % len(users)],  # Assign a user to the recipe
            image_url=image_urls[i]  # Assign an image URL to the recipe
        )
        # Add the first 3 ingredients and 2 categories
...
# Setup commands to insert test data
def setup_commands(app):
    ...
    def insert_test_data():
        ...
        # Insert test data
        insert_test_users()         # Insert users
        insert_test_categories()    # Insert categories
        insert_test_ingredients()   # Insert ingredients
        insert_test_recipes()       # Insert recipes
